...
export default function RegulaFalsiPage() {
  ...
  const handleSolve = async () => {
    try {
      const response = await axios.post("http://localhost:8000/api/regula-falsi", {
  ...
  return (
    <div className="page-layout" style={{ display: 'flex', gap: '20px' }}>
      <div className="input-panel" style={{ width: '300px', background: '#161B22', padding: '20px' }}>
        <h3 style={{color: '#FFA657'}}>Regula Falsi</h3>
      ...
      <div className="graph-panel" style={{ flexGrow: 1, height: '600px' }}>
        <ResultGraph result={result} equation={equation} methodColor="#FFA657" />
      </div>
    </div>
  );
}